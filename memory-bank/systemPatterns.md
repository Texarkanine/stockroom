...
Harness hooks are **not** the same JSON shape or event: Cursor native hooks (`hooks/cursor-hooks.json`) use flat `{ "version": 1, "hooks": { "sessionStart": […], "beforeSubmitPrompt": […] } }` per [Cursor Hooks](https://cursor.com/docs/hooks); Claude Code hooks (`hooks/claude-hooks.json`) keep nested `SessionStart` / `hooks[]` / `type: "command"`. Do not copy Claude structure into the Cursor file. Both harnesses bootstrap session-start rectify with `uv python find --project … --no-config` (not bare `python3`, not `uv run --no-sync`). Cursor also registers a trimmed non-blocking `beforeSubmitPrompt` suspenders path (`continue` JSON first, then background `shim rectify --path-only` — no ensure-env, no dashboard); full ensure + dashboard stay on `sessionStart`. Cursor plugin hooks also require **Include third-party Plugins, Skills, and other configs** until [plugin hooks not loading](https://forum.cursor.com/t/plugin-hooks-not-loading-into-cursor-ide/156702) is fixed.
...
Human user-guide / architecture / contributor prose lives under [`docs/`](../docs/) (properdocs site). Agents use `SKILL.md` + [`system-model.md`](../skills/sr-search/references/system-model.md) only — do not park a user-guide corpus under `skills/**/references/`. Recipe SSOT for gnarly `sr-query` starters lives under [`skills/sr-query/references/cookbook/`](../skills/sr-query/references/cookbook/); Advanced docs keep a human-owned cookbook index and expose each recipe body via symlink under [`docs/advanced/cookbook/`](../docs/advanced/cookbook/), but agents still do not use the docs site as runtime input. Root `pyproject.toml` is a docs-toolchain stub; the engine remains under `skills/sr-search/`. Using-agent doctrines (`system-model.md`) and this maintainer briefing are related but not one SSOT — do not merge them. Contributor enter/verify/exit is [`docs/contributing/preparation.md`](../docs/contributing/preparation.md); day-to-day engine / Torch / docs / dashboard / skills loops and Make targets are [`docs/contributing/iteration/index.md`](../docs/contributing/iteration/index.md).
