...
- [x] **`sr-semantic`** — a pure vector-search read surface: embed the query, run cosine KNN over the HNSW index, join back to the owner rows, and print ranked results read-only through the chokepoint.
