...
- New test file: `skills/sr-search/tests/test_query_cookbook.py` (structural discoverability + wiring).
...
1. **TDD: stub failing cookbook wiring tests**
    - Files: `skills/sr-search/tests/test_query_cookbook.py`
    - Changes: assert expected paths exist; `SKILL.md` contains cookbook index link; `docs/advanced/cookbook.md` contains `--8<--` markers for each recipe; nav mentions Cookbook. Run → fail.
...
2. **Cookbook SSOT: index + recipes**
    ...
    - Changes: short dual-audience recipes. Ground SQL in:
        ...
        - Skills Claude: user `<command-name>` via `regexp_extract` + builtin `NOT IN` list aligned with `_CLAUDE_BUILTIN_COMMANDS`; agent `tool_name = 'Skill'` + `json_extract_string(..., '$.skill')`; exclude skill-info blob prefix; document drift trigger → `skill_usage.py` / dashboard tests.
...
3. **Agent discoverability + thoughtful SKILL edits**
...
4. **Human docs: advanced cookbook + nav + user-guide pointer**
    ...
    - Changes: human when/why + promotion/drift note; `--8<--` include each recipe from skill paths; note SSOT path for editors; nav entry “Cookbook”; search.md points at Advanced cookbook for gnarly starters.
...
6. **Make tests pass + verify docs**
    - Run `test_query_cookbook.py` → green; full `make test` (or project equivalent); `make docs-build` strict green.
    - Manual smoke (if warehouse present): run one tools + one skills-claude + one token recipe via `stockroom query`.

7. **systemPatterns surgical note (only if needed)**
    - Files: `memory-bank/systemPatterns.md`
    - Changes: only if “docs ownership” paragraph would be factually wrong after snippets-in cookbook — one sentence that cookbook recipe bodies live under `sr-query/references/cookbook/` and may be snippet-included into Advanced docs. Skip if still accurate enough.
