...
8. **`__init__.ingest` orchestrator** — RED: full fixture ingest via injected `con`; counts match; incremental no-op on advanced watermark; cross-file subagent joins. GREEN: wire stages; open `warehouse.open(RW)` when `con` is None; apply enrichment.
   - Files: `src/stockroom/ingest/__init__.py`, `tests/test_ingest_orchestrator.py`.
