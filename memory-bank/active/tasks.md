...
- **Make / CI**: new `schema-docs` / `schema-docs-check` targets; engine CI and `make ci` run the check. Docs site CI stays a strict properdocs build (mermaid fence already enabled in `properdocs.yaml`).
...
1. Stub tests: empty cases in `test_warehouse_schema_docs.py` for toy render, type sanitization, view heuristic, relationship lines, head-snapshot coverage, `--check` fail-when-missing (tmp_path), write-then-check (tmp_path).
...
3. Write tests and run red: assertions on mermaid `erDiagram` text, sanitized `FLOAT[384]`, empty-PK view marking, explicit relationship strings, every head-snapshot table name present, `check` nonzero on missing/mismatch, `write` then `check` ok. Run `cd skills/sr-search && uv run --no-sync --no-config pytest -n0 tests/test_warehouse_schema_docs.py -v` (expect FAIL).
4. Write code and run green: stdlib JSON + pathlib; discover max `NNNN_snapshot.json`; emit markdown template (title, "logical relationships / no DB FKs", mermaid fence, short index note); explicit `RELATIONSHIPS` list (`sessions`→`messages`, `messages`→`tool_calls`, `sessions`→`session_token_usage`, polymorphic `embeddings` from `messages` / `tool_calls` via `owner_table`); CLI `python3 scripts/gen_warehouse_schema.py` writes, `--check` compares. Re-run the same pytest until green.
...
- Files: `docs/advanced/warehouse-schema.md` (symlink), `skills/sr-search/tests/test_warehouse_schema_docs.py`
...
4. Write code and run green: create the symlink; pytest green.
...
4. Write code and run green: `schema-docs` → `python3 scripts/gen_warehouse_schema.py`; `schema-docs-check` → `python3 scripts/gen_warehouse_schema.py --check`; add `schema-docs-check` to `make ci`; pytest green.
...
- Files: `docs/advanced/.pages`, `docs/advanced/index.md`, `docs/advanced/duckdb.md`, `docs/user-guide/search.md`, `docs/architecture/warehouse.md`
- No tests: prose/policy artifact

1. Nav: add Warehouse schema next to Cookbook under Advanced.
2. Advanced index + DuckDB + Search: link the picture for query-forming.
3. Architecture warehouse Migrations section: route to Advanced for the ERD; do not paste DDL or the mermaid into Architecture (inclusion bar).

### 8. Contributor regen loop — prose/policy

- Files: `docs/contributing/iteration/engine.md` and/or `docs/contributing/iteration/docs.md` (whichever already owns schema-golden workflow; engine tests own `STOCKROOM_UPDATE_SCHEMA_GOLDEN` — put the two-step loop there, one line from docs.md if needed)
- No tests: prose/policy artifact

1. Document: schema change → update head golden (`STOCKROOM_UPDATE_SCHEMA_GOLDEN=1`) → `make schema-docs` → commit both. If a new table participates in joins, update `RELATIONSHIPS` in the generator.
...
- **Two-step contributor workflow** (golden then ERD): CI fails if either is skipped; Make target is grep-able; contributing docs name both.
