...
- **Tokens**: sum allowlisted `tokenCount.inputTokens` / `outputTokens` across *all* of the composer's bubbles (not only kept ones), and set the session-grain fields only when a sum is nonzero. Message-grain token fields stay NULL.
