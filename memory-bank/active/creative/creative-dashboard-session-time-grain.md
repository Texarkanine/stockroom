...
- Writer inserts the new column; golden `expected_rows.json` regenerates via the test's own helper (snapshot discipline). Fixture mtimes must be pinned with `os.utime` in the test so the golden stays machine-independent.
