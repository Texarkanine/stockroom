...
- **Makefile** — root [`Makefile`](../Makefile) for sync/lock/test/lint/format/reuse/ci/torch/shim/docs plus contributor localdev atoms (`localdev`, `localdev-clean`, `localdev-status`, `plugin-local`, `shim TAKEOVER=1`).
