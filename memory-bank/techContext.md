...
- **Makefile** — root [`Makefile`](../Makefile) for sync/lock/test/lint/format/reuse/ci/torch/shim/docs plus contributor localdev (`localdev`, `localdev-clean`, `localdev-status`, `shim` with optional `TAKEOVER=1` / `FORCE=1`).
