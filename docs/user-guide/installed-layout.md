...
| `$XDG_CONFIG_HOME/stockroom/config.toml` (or `~/.config/stockroom/config.toml` by default) | **Optional settings:** distinct from data home; today additive Cursor `ai_tracking_dbs` pins for model enrichment ([Load the Warehouse](ingest/index.md#cursor-sessionsmodels-enrichment)) and `state_vscdb`, the legacy store path for [backfill](ingest/backfill/cursor-vscdb.md#pointing-at-the-store) |
