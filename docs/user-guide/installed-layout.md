...
| `$XDG_CONFIG_HOME/stockroom/config.toml` (or `~/.config/stockroom/config.toml` by default) | **Optional settings:** distinct from data home; today additive Cursor `ai_tracking_dbs` pins for model enrichment ([Harness Sources](load/sources.md#cursor-sessionsmodels-enrichment)) and `state_vscdb`, the legacy store path for [backfill](load/backfill/cursor-vscdb.md#pointing-at-the-store) |
