...
Torch is required to encode; it is held out of the lock for machine-specific builds — see [Packaging](packaging.md).
...
Both read surfaces print only through one render chokepoint. `--detail` (compact / snippet / full / raw) is orthogonal to `--format` (tsv default; json / table opt-in). Truncation markers are display bounds, not warehouse mutation — see [Warehouse](warehouse.md).
