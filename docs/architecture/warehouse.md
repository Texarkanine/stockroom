...
Rebuildable projection of harness history — not the system of record — and read surfaces that cannot write by construction.
...
Kept content lands in shared tables (`sessions`, `messages`, `tool_calls`, `embeddings`, `_sync_state`). Prompts and responses are stored whole; tool *inputs* are kept; tool *result* payloads are dropped. Thinking/reasoning blocks the harness keeps separate are not stored. There is no raw mirror layer beside the typed model.
...
Fidelity doctrines: whole kept fields, uniform identity, honest workspace paths.
...
Connection chokepoints, concurrency, UTC at rest, and forward-only migrations.
...
Migrations are numbered forward-only SQL under the engine’s `migrations/` tree; `schema_version` is runner-owned. Schema changes go through that chokepoint — Architecture does not list DDL here.
